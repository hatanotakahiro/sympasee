...
メールアドレス: aaa@aaa
パスワード: aaa111
にてログインした状態で、こちらのURL（http://54.95.42.129/）にアクセスするとご覧いただけます。


他ユーザのマイページにアクセスすると、そのユーザーがこれまでに出品してきた商品や、自己紹介を確認することができます。このページは、商品出品ページにおける「出品者」のボタンをクリックすると見ることができます。

この機能によって、「つながりたいユーザーであるかどうか」を判断する材料が増えます。

実装にあたって工夫した点は、ログインしているユーザー自身のページである場合には、その場で編集ができるようにした点です。なるべくページを分けることはせず、シンプルな設計であることを意識しました。

現状、ユーザーのアイコン写真を登録するこができていません。ユーザーを認知するためには、アイコン画像も重要であると考えます。今後、マイページからアイコン画像の登録・編集をできるように追加実装したいと考えています。

## レーティング機能
ログインした状態で、こちらのURL（http://54.95.42.129/movies/1/reviews/new）にアクセスするとご覧いただけます。

## フォローフォロワー機能
同じくユーザー詳細表示ページから確認することができます（http://54.95.42.129/users/8）。


他ユーザーの詳細ページに遷移し「フォローする」をクリックすると、ユーザーをフォローすることができます。フォローしたユーザーは、自身のマイページ下部から確認することができます。

この機能によって「自身と相性が良いユーザーにすぐにアクセスできる」という利点があります。

実装にあたって工夫した点は、フォローボタンを非同期通信にしたことです。画面がリロードされることなく、使い勝手が良くなるように意識しました。

現状、フォローしているユーザーと直接コミュニケーションをとることができません。追加実装として「相互フォローであればダイレクトメッセージでコミュニケーションがとれる」という機能を追加したいと考えています。
